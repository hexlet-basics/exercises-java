...
Запись `String...` означает «ноль или больше строк». Обязательные параметры (как `separator`) идут первыми, а переменная часть — всегда последней. Внутри метода эти аргументы доступны как обычный массив, поэтому по ним можно пройтись и что-то с ними сделать.
