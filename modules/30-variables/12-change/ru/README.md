...
Здесь мы сначала записали в переменную одну строку (_Father!_), потом другую (_Mother!_). Имя переменной не изменилось, но значение внутри стало другим. Ключевое слово `var` нужно только при создании переменной. При изменении значения его уже не пишут.
